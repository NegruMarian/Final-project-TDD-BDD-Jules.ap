...
def step_impl(context):
    context.sign_in_page.clear_pass()
@then("sign in: I verify error- Please enter your password!")   # rezolvare tema 12 ex1.
def step_impl(context):
    context.sign_in_page.check_error_pass()
@when('sign in : I check the login button is disabled')          # rezolvare tema 12 ex1.
def step_impl(context):
    context.sign_in_page.check_login_button_is_disabled()
